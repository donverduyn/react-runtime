...
describe('strict mode/hooks', () => {
  ...
  //   expect(symbol1).toStrictEqual(symbol3);
  //   expect(symbol1).not.toStrictEqual(symbol2);
  // });
  it('should reuse the idRef across renders', () => {
    ...
    expect(ids).toStrictEqual([1, 1, 2, 1]);
  });
  ...
  it('should match function calls in React.StrictMode', () => {
    ...
    // const callbackCalls: [] = [];
    // expect(callbackFn.mock.calls.flatMap(identity)).toStrictEqual(callbackCalls);

    const renderCalls = ['root', 'root', 'child', 'child'];
    expect(renderFn.mock.calls.flatMap(identity)).toStrictEqual(renderCalls);

    const effectCalls = ['child', 'root', 'child', 'root'];
    expect(effectFn.mock.calls.flatMap(identity)).toStrictEqual(effectCalls);

    const unmountCalls = ['root', 'child'];
    expect(unmountFn.mock.calls.flatMap(identity)).toStrictEqual(unmountCalls);
  });
