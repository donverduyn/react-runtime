...
//   private instances = new Map<string, RuntimeInstance2<any>>();
//   // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
//   private createTimeout<T>(config: Config, instance: RuntimeInstance2<T>) {
...
//       if (config.replace && existingInstance && !existingInstance.isDisposed) {
//         this.log(
//           config,
...
//         if (config.cleanupPolicy === 'dispose') {
//           setTimeout(() => {
...
//   if (!store) {
//     throw new Error(`[useRuntimeInstance] Store is not initialized.`);
//   }
